...
//public static final String WWW_AUTHENTICATE = "WWW-Authenticate";
			...
			//headers.set("Referer", "http://kind.krx.co.kr/corpgeneral/corpList.do?method=loadInitPage");
			headers.set("Host", "203.235.1.50");
			headers.set("Origin", "http://203.235.1.50");
			headers.set("Referer", "http://203.235.1.50/corpgeneral/corpList.do?method=loadInitPage");
			headers.set("User-Agent", "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36");
			headers.set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.120 Safari/537.36 NetHelper70");
...
//                    String.class);
	...
	public void excelDownloadUsingByteArrayHttpMessageConverter(String marketType) {
		try {
			...
			headers.set("Accept", MediaType.APPLICATION_JSON + ",text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3");
			headers.set("Accept-Encoding", "Accept-Encoding: gzip, deflate");
			headers.set("Accept-Language", "en-US,en;q=0.9,ko;q=0.8");
...
//method=download&pageIndex=1&currentPageSize=5000&comAbbrv=&beginIndex=&orderMode=3&orderStat=D&isurCd=&repIsuSrtCd=&searchCodeType=&marketType=kosdaqMkt&searchType=13&industry=&fiscalYearEnd=all&comAbbrvTmp=&location=all
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(SERVER_URI)
				...
				.queryParam("pageIndex", "1")
				.queryParam("currentPageSize", "5000")
				.queryParam("comAbbrv", "")
				.queryParam("beginIndex", "")
				.queryParam("orderMode", "3")
				.queryParam("orderStat", "D")
...
//		        ResponseEntity<String> response = restTemplate.exchange(builder.toUriString(), HttpMethod.POST, entity, String.class);
			...
			System.out.println("Response Headers[Content-Description] :" + response.getHeaders().get("Content-Description"));
			System.out.println("Response Headers[Content-Disposition] :" + response.getHeaders().get("Content-Disposition"));
...
//				Files.write(Paths.get(marketType + "_excelDownload.xls"), response.getBody());
	...
	public void excelDownloadUsingStringHttpMessageConverter2(String marketType) throws Exception {
		...
		URI targetUri = UriComponentsBuilder.fromHttpUrl(SERVER_URI)
			.queryParam("method", "download")
			.queryParam("pageIndex", "1")
			.queryParam("currentPageSize", "5000")
			.queryParam("comAbbrv", "")
...
//		headers.set("User-Agent", "mozilla");
		...
		ResponseEntity<String> response = restTemplate.exchange(targetUri.toURL().toString(), HttpMethod.GET, entity, String.class);
...
//		Connection conn = Jsoup.connect(strUri).cookie("cookiereference", "cookievalue").method(Method.POST);
		...
		headers.put("Accept", MediaType.APPLICATION_JSON + ",text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3");
...
//		headers.put("Content-Type", "application/xhtml+xml; charset=EUC-KR");
		...
		headers.put("Cookie", "__smVisitorID=QxeY65c5t3z; JSESSIONID=NyCFzfuTJuLCu1YTU5tAy2RDQUIha813iVKfZ9cnDZKOG81CUOKWwLcMsKQsK6JP.amV1c19kb21haW4vMTBfRFNUMg==; viewMode=1; krxMenu=ULDDST00000%2C%uC624%uB298%uC758%uACF5%uC2DC/ULDDST00100%2C%uD68C%uC0AC%uBCC4%uAC80%uC0C9/ULDDST00300%2C%uC0C1%uC138%uAC80%uC0C9/ULDDST00200%2C%uD1B5%uD569%uAC80%uC0C9/ULDDST71000%2C%uC608%uBE44%uC2EC%uC0AC%uAE30%uC5C5/");
		//headers.put("Host", "kind.krx.co.kr");
		//headers.put("Origin", "http://kind.krx.co.kr");
		//headers.put("Referer", "http://kind.krx.co.kr/corpgeneral/corpList.do?method=loadInitPage");
...
//		headers.put("User-Agent", "mozilla");
		...
		headers.put("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.120 Safari/537.36 NetHelper70");
...
//		Jsoup.connect(url).requestBody(json).header("Content-Type", "application/json").post();
		Document doc = Jsoup.connect(strUri)
			.requestBody("JSON")
...
//		Files.write(Paths.get(marketType + "_excelDownload.xls"), html.getBytes("EUC-KR"));
		Files.write(Paths.get(marketType + "_3_excelDownload.xls"), html.getBytes());
