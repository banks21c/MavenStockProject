...
public final class OpenApiStaticApplication {

	private static org.slf4j.Logger logger = LoggerFactory.getLogger(OpenApiStaticApplication.class);
...
//    private final static String REQUEST_JSON = "{\"coupangUrls\": [\"https://www.coupang.com/np/campaigns/82\"]}";
	...
	OpenApiStaticApplication() {
//		getShortenedUrl("https://www.coupang.com/np/coupangglobal",ACCESS_KEY,SECRET_KEY);
		getReportsClicks("20200501", "20200630",ACCESS_KEY,SECRET_KEY);
		getReportsOrders("20200501", "20200630",ACCESS_KEY,SECRET_KEY);
		getReportsCancels("20200501", "20200630",ACCESS_KEY,SECRET_KEY);

		//카테고리 별 베스트 상품에 대한 상세 상품 정보를 생성합니다.
		getBestcategoryProducts(ACCESS_KEY, SECRET_KEY);
		getGoldboxProducts(ACCESS_KEY, SECRET_KEY);
		//쿠팡 PL 상품에 대한 상세 정보를 생성합니다.
		//COUPANG_PL_URL = API_PATH + "​​/products​/coupangPL";
		getCoupangPLProducts(ACCESS_KEY, SECRET_KEY);
		//쿠팡 PL 브랜드별 상품 상세 정보를 생성합니다.
		getCoupangPLBrandProducts(ACCESS_KEY, SECRET_KEY);
		//검색 키워드에 대한 쿠팡 검색 결과와 상세 상품 정보를 생성합니다 (1 시간당 최대 10번 호출 가능합니다.)
		getSearchProducts("egg+slicer",ACCESS_KEY, SECRET_KEY);
	}

	OpenApiStaticApplication(String accessKey, String secretKey) {
		ACCESS_KEY = accessKey;
		SECRET_KEY = secretKey;
	}

	public static String getShortenedUrl(String normalUrl,String accessKey, String secretKey) {
		logger.debug("normalUrl :" + normalUrl);
		try {
			Thread.sleep(2000);
		} catch (InterruptedException ex) {
			Logger.getLogger(OpenApiStaticApplication.class.getName()).log(Level.SEVERE, null, ex);
		}
		...
		String authorization = HmacGenerator.generate(REQUEST_METHOD_POST, DEEPLINK_URL, accessKey, secretKey);
	...
	public static String getReportsClicks(String startDate, String endDate,String accessKey,String secretKey) {
		String strParamJson = "";
		String reports_url = REPORTS_CLICKS_URL + "?startDate=" + startDate + "&endDate=" + endDate;
		return getData("클릭상품", reports_url, "", strParamJson,accessKey,secretKey);
	}

	public static String getReportsOrders(String startDate, String endDate,String accessKey,String secretKey) {
		String strParamJson = "";
		String reports_url = REPORTS_ORDERS_URL + "?startDate=" + startDate + "&endDate=" + endDate;
		return getData("주문상품", reports_url, "", strParamJson,accessKey,secretKey);
	}

	public static String getReportsCancels(String startDate, String endDate,String accessKey,String secretKey) {
		String strParamJson = "";
		String reports_url = REPORTS_CANCELS_URL + "?startDate=" + startDate + "&endDate=" + endDate;
		return getData("취소상품", reports_url, "", strParamJson,accessKey,secretKey);
	}

	//카테고리별 베스트 상품에 대한 상세 상품 정보를 생성합니다.
	//BESTCATEGORIES_URL = API_PATH + "​/products​/bestcategories​/{categoryId}";
	public static boolean getBestcategoryProducts(String categoryId, String categoryNm,String accessKey,String secretKey) {
		...
		String data = getData("카테고리별 베스트상품", bestcategoriesUrl, categoryNm, strParamJson,accessKey,secretKey);
		sb.append(data);
		try {
			Thread.sleep(5000);
		} catch (InterruptedException ex) {
	...
	public static StringBuilder getBestcategoryProducts(int idx, String categoryId, String categoryNm, int limit,String accessKey,String secretKey) {
		String bestcategoriesUrl = "";
		StringBuilder sb = new StringBuilder();
	...
	public static boolean getBestcategoryProducts(String accessKey, String secretKey) {
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		...
		for (int i = 0; i < bestCategoriesArray.length; i++) {
			...
			if (codeValue.length == 2) {
				...
				sb.append(getBestcategoryProducts(i, categoryId, categoryNm, limit,accessKey,secretKey));
			}
		}
		sb.append("<div>※ 파트너스 활동을 통해 일정액의 수수료를 제공받을 수 있음</div>");
	...
	public static boolean getGoldboxProducts(String accessKey,String secretKey) {
		StringBuilder sb = new StringBuilder();
		sb.append("<div style='width:100%;padding-top:20px;float:left;'><h1>").append(strYmd).append(" ").append("골드박스 상품").append("</h1></div>");
		String strParamJson = "";
		System.out.println("strParamJson:" + strParamJson);
		String data = getData("골드박스 상품", GOLDBOX_URL, "", strParamJson,accessKey,secretKey);
	...
	//쿠팡 PL 상품에 대한 상세 정보를 생성합니다.
	//COUPANG_PL_URL = API_PATH + "​​/products​/coupangPL";
	public static boolean getCoupangPLProducts(String secretKey, String accessKey) {
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		...
		System.out.println("strParamJson:" + strParamJson);
		String data = getData("쿠팡PL상품", COUPANG_PL_URL, "", strParamJson,accessKey,secretKey);
		sb.append(data);
		sb.append("<div>※ 파트너스 활동을 통해 일정액의 수수료를 제공받을 수 있음</div>");
	...
	public static boolean getCoupangPLBrandProducts(String brandId, String brandNm,String accessKey,String secretKey) {
		String server_url = "";
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		...
		String strParamJson = "";
		String data = getData("쿠팡PL브랜드상품", server_url, brandNm, strParamJson,accessKey,secretKey);
		sb.append(data);
	...
	public static StringBuilder getCoupangPLBrandProducts(int idx, String brandId, String brandNm, int limit,String accessKey,String secretKey) {
		String server_url;
		StringBuilder sb = new StringBuilder();
	...
	public static boolean getCoupangPLBrandProducts(String accessKey,String secretKey) {
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		...
		for (int i = 0; i < coupangPlBrandArray.length; i++) {
			...
			if (codeValue.length == 2) {
				...
				logger.debug("brandNm:"+brandNm);
				sb.append(getCoupangPLBrandProducts(i, brandId, brandNm, limit,accessKey,secretKey));
			}
		}
		sb.append("<div>※ 파트너스 활동을 통해 일정액의 수수료를 제공받을 수 있음</div>");
	...
	public static boolean getSearchProducts(String keyword,String accessKey,String secretKey) {
		StringBuilder sb = new StringBuilder();
		...
		String searchUrl = SEARCH_URL + "?keyword=" + encodedKeyword + "&limit=" + limit;
//		server_url = server_url + "?keyword=food&limit=" + limit;
		logger.debug("server_url:" + searchUrl);
		String data = getData("상품검색", searchUrl, "", strParamJson,accessKey,secretKey);
		sb.append(data);
		sb.append("<div>※ 파트너스 활동을 통해 일정액의 수수료를 제공받을 수 있음</div>");
...
//		FileUtil.fileWrite(strFileName, sb.toString());
	...
	public static String getData(String apiGubun, String server_url, String categoryNm, String strParamJson,String accessKey,String secretKey) {
		System.out.println("apiGubun :" + apiGubun);
		...
		StringBuilder sb = new StringBuilder();
		// Generate HMAC string
		String authorization = HmacGenerator.generate(REQUEST_METHOD_GET, server_url, accessKey, secretKey);
