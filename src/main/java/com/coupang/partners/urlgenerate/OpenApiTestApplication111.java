...
public final class OpenApiTestApplication111 {

	private static org.slf4j.Logger logger = LoggerFactory.getLogger(OpenApiTestApplication111.class);
...
//		getCoupangPLBrandProducts(COUPANG_PL_BRAND_URL);
	...
	public static String getShortenedUrl(String normalUrl) {
		...
		try {
			...
			Logger.getLogger(OpenApiTestApplication111.class.getName()).log(Level.SEVERE, null, ex);
		}
	...
	public void getBestcategoryProducts(String bestcategoriesUrl) {
		...
		for (int i = 0; i < bestCategoriesArray.length; i++) {
			...
			if (codeValue.length == 2) {
				...
				String data = getProductData("카테고리별 베스트상품", bestcategoriesUrl, categoryNm, strParamJson);
				sb.append(data);
				try {
					Thread.sleep(5000);
	...
	public void getGoldboxProducts(String server_url) {
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		sb.append("<div style='width:100%;'><h1>").append(strYmd).append(" ").append("골드박스 상품").append("</h1></div>");
		String strParamJson = "";
		System.out.println("strParamJson:" + strParamJson);
	...
	public void getCoupangPLProducts(String server_url) {
		int limit = 20;
		StringBuilder sb = new StringBuilder();
		sb.append("<div style='width:100%;'><h1>").append(strYmd).append(" ").append("쿠팡 PL 상품").append("</h1></div>");
	...
	public void getCoupangPLBrandProducts(String server_url) {
		...
		for (int i = 0; i < coupangPlBrandArray.length; i++) {
			...
			if (codeValue.length == 2) {
				...
				String strParamJson = "";
				String data = getProductData("쿠팡PL브랜드상품", server_url, brandNm, strParamJson);
				sb.append(data);
				try {
					Thread.sleep(5000);
				} catch (InterruptedException ex) {
	...
	public String getSearchProducts(String server_url, String keyword) {
		...
		String strParamJson = "";
		int limit = 20;
		server_url = server_url + "?keyword=" + keyword + "&limit=" + limit;
		String data = getData("상품검색", server_url, strParamJson);
		sb.append(data);
		sb.append("<div>※ 파트너스 활동을 통해 일정액의 수수료를 제공받을 수 있음</div>");

		strFileName = USER_HOME + File.separator + "documents" + File.separator + strDate + "_상품검색.html";
		FileUtil.fileWrite(strFileName, Jsoup.parse(sb.toString()).html());
//		FileUtil.fileWrite(strFileName, sb.toString());		
		return sb.toString();
	}

	public String getProductData(String apiGubun, String server_url, String categoryNm, String strParamJson) {
...
//							System.out.println("obj2:" + obj2.toString());
		...
		strFileName = USER_HOME + File.separator + "documents" + File.separator + strDate + "_" + apiGubun + ".html";
		FileUtil.fileWrite(strFileName, Jsoup.parse(sb.toString()).html());
//		FileUtil.fileWrite(strFileName, sb.toString());
		return sb.toString();
	}

	public String getData(String apiGubun, String server_url, String strParamJson) {
		System.out.println("server_url :" + server_url);
		StringBuilder sb = new StringBuilder();
		// Generate HMAC string
		String authorization = HmacGenerator.generate(REQUEST_METHOD_GET, server_url, ACCESS_KEY, SECRET_KEY);
...
//				System.out.println("value:" + obj.toString());
				...
				if (key.equals("data")) {
					JSONArray data = (JSONArray) jsonObject.get("data");
					logger.debug("data.length:" + data.length());
					sb.append("<div style='width:100%;'><h1>").append(strYmd).append(" ").append(apiGubun).append("</h1></div>");
					sb.append("<div style='width:100%;float:left;'>");
					sb.append("<ul style='list-style:none;padding-left:0'>");
					for (int i = 0; i < data.length(); i++) {
						JSONObject dataObj2 = (JSONObject) data.get(i);
...
//						sb.append("<li style='float:left;width:250px;height:484px;'>");
						sb.append("<li style='float:left;width:250px;height:390px;background-color: #fff; box-shadow: none; border: 1px solid #dfe1e5; border-radius: 8px; overflow: hidden; margin: 0 0 6px 0;margin-right:8px;margin-top:1px;padding:5px 10px;'>");
						while (it2.hasNext()) {
							String key2 = (String) it2.next();
//							System.out.println("key2:" + key2);
