...
public class StockCronTriggerExample {
	...
	public StockCronTriggerExample() throws Exception {
...
//		job = newJob(StockPlusMinusDivide100Job.class).withIdentity("job0", "group0").build();
		...
		trigger = newTrigger().withIdentity("trigger0", "group0").withSchedule(cronSchedule("0 */1 * ? * *")).build();
		sched.scheduleJob(job, trigger);

		// Job #1 is scheduled to run every day at 15:40pm
		job = newJob(StockPlusMinusDivide100Job.class).withIdentity("job1", "group1").build();
		trigger = newTrigger().withIdentity("trigger1", "group1").withSchedule(cronSchedule("0 40 15 * * ?")).build();
		sched.scheduleJob(job, trigger);
