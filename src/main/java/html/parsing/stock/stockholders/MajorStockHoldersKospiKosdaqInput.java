	...
	public void writeFile(List<StockVO> list, String title) {
		...
		for (StockVO svo : list) {
			...
			if (svo != null) {
				...
				sb2.append("<td rowspan=" + listSize + "><a href='" + url + "' target='_blank'>" + svo.getStockName()
						+ "</a></td>\r\n");
				...
				for (int i = 0; i < listSize; i++) {
					...
					if (svo.getSpecialLetter().equals("+")) {
						...
						sb2.append("<td style='text-align:right;color:red;'>" + svo.getSign() + svo.getVaryPrice()
								+ "%</td>\r\n");
						sb2.append("<td style='text-align:right;color:red;'>" + svo.getSpecialLetter()
								+ svo.getVaryRatio() + "%</td>\r\n");
					} else if (svo.getSpecialLetter().equals("-")) {
						sb2.append("<td style='text-align:right;color:blue;'>" + svo.getCurPrice() + "</td>\r\n");
						sb2.append("<td style='text-align:right;color:blue;'>" + svo.getSign() + svo.getVaryPrice()
								+ "</td>\r\n");
						sb2.append("<td style='text-align:right;color:blue;'>" + svo.getSpecialLetter()
								+ svo.getVaryRatio() + "%</td>\r\n");
					} else {
						sb2.append("<td style='text-align:right;color:gray;'>" + svo.getCurPrice() + "</td>\r\n");
						sb2.append("<td style='text-align:right;color:gray;'>" + svo.getSign() + svo.getVaryPrice()
								+ "%</td>\r\n");
						sb2.append("<td style='text-align:right;color:gray;'>" + svo.getSpecialLetter()
								+ svo.getVaryRatio() + "%</td>\r\n");
					}
		...
		sb1.append("<div style='width:741px;text-align:right;font-weight:bold;'>국민연금 총 보유액?<span style='color:" + sColor
				+ "'>" + new DecimalFormat("#,##0").format(lRetainAmount / 1000000) + "(백만)</span></div>\r\n");
		sb1.append(
				"<div style='width:741px;text-align:right;font-weight:bold;'>국민연금이 오늘 번 돈은?<span style='color:" + sColor
						+ "'>" + new DecimalFormat("#,##0").format(lEarningAmount / 1000000) + "(백만)</span></div>\r\n");
