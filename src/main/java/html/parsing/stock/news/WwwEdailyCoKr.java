...
public class WwwEdailyCoKr extends News implements NewsInterface {

	private static Logger logger = LoggerFactory.getLogger(WwwEdailyCoKr.class);
	...
	public StringBuilder createHTMLFileCommon(Document doc, String strUrl, String strMyComment) {
		...
		if (article.isEmpty()) {
			...
			article.select(".news_titles").remove();
		}
		if (strUrl.contains("realtimenews")) {
