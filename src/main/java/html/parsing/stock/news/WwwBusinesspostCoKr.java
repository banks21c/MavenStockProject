...
public class WwwBusinesspostCoKr extends News implements NewsInterface {
	...
	WwwBusinesspostCoKr(int i) {
		logger = LoggerFactory.getLogger(this.getClass());

		String url = JOptionPane.showInputDialog(this.getClass().getSimpleName() + " URL을 입력하여 주세요.");
		System.out.println("url:[" + url + "]");
		if (url == null || url.equals("")) {
			url = "http://www.businesspost.co.kr/BP?command=mobile_view&num=189360";
		}
		createHTMLFile(url);
	}
	...
	public StringBuilder createHTMLFile(String url, String strMyComment) {
		...
		try {
			...
			strTitle = doc.select(".article_header .article_tit").text();
			...
			Elements article = doc.select(".post-contents");
			if (article.size() <= 0) {
