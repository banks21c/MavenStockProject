...
public class NewsDaumNet extends News implements NewsInterface {

	private static Logger logger = LoggerFactory.getLogger(NewsDaumNet.class);
	...
	public StringBuilder createHTMLFile(String url, String strMyComment) {
		...
		try {
			...
			Elements timeElements = doc.select(".head_view .info_view .txt_info");
			Element timeElement = null;
			if (timeElements.size() > 0) {
				timeElement = doc.select(".head_view .info_view .txt_info").get(1);
