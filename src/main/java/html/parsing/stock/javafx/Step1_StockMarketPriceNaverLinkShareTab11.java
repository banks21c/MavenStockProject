					...
					public void handle(javafx.scene.input.MouseEvent event) {
						try {
							...
							java.util.logging.Logger
									.getLogger(Step1_StockMarketPriceNaverLinkShareTab11.class.getName())
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (item.contains(":")) {
				...
				if (strCategoryArray.length > 1) {
					strCategoryNo = strCategoryArray[0];
					strCategoryName = strCategoryArray[1];
					System.out.println("strCategoryNo-------------->" + strCategoryNo);
					System.out.println("strCategoryName-------------->" + strCategoryName);
				}
				if (categoryName.equals(strCategoryName)) {
					categoryNo = strCategoryNo;
					break;
				}
			}
		}
