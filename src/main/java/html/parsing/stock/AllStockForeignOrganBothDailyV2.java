...
public class AllStockForeignOrganBothDailyV2 {

	final static String USER_HOME = System.getProperty("user.home");
	private static Logger logger = LoggerFactory.getLogger(AllStockForeignOrganBothDailyV2.class);
	...
	public static void main(String[] args) {
		new AllStockForeignOrganBothDailyV2();
	}

	AllStockForeignOrganBothDailyV2() {
//		test();
		real();
	}
...
//			svo = getStockTrade(svo);
	...
	public static List<StockVO> getAllStockList(List<StockVO> stockList, String buyOrSell) {
		...
		for (StockVO svo : stockList) {
			...
			long lForeignOrganTradingAmount = svo.getlForeignOrganTradingAmount();

			if (buyOrSell.equals("buy")) {
				if (lForeignOrganTradingAmount > 0) {
					stocks.add(svo);
				}
			} else {
				if (lForeignOrganTradingAmount < 0) {
	...
	public void writeFile(String title) {
		try {
			...
			sb1.append("\t<div><font size=3>").append(strDailyOrWeekly + "코스피 외인,기관 양매수,양매도 거래대금").append("</font><div>\r\n");
			sb1.append("<table>\r\n");
			sb1.append("<tr>\r\n");
			sb1.append(
			...
			sb1.append("\t<div><font size=3>").append(strDailyOrWeekly + "코스닥 외인,기관 양매수,양매도 거래대금").append("</font></div>");
			sb1.append("<table>\r\n");
			sb1.append("<tr>\r\n");
			sb1.append(
					"<td rowspan='2' style='background:#669900;color:#ffffff;text-align:center;font-size:12px;'>No.</td>\r\n");
