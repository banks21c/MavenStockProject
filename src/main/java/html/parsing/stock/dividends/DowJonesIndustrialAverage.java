...
//public static final String SERVER_URI = "https://www.nyse.com/listings_directory/stock";
	...
	DowJonesIndustrialAverage() {
		...
		try {
			...
			java.util.logging.Logger.getLogger(DowJonesIndustrialAverage.class.getName()).log(Level.SEVERE, null, ex);
		}
		/*
		 * //S&P 500 List<StockVO> sAndP500List = getUsStockList(sAndP500, "sAndP500");
		 * Collections.sort(sAndP500List, new DividendRateDescCompare());
		 * writeUsStockList(sAndP500List, "dowJones"); logger.debug("sAndP500List.size:"
		 * + sAndP500List.size()); //나스닥 100 List<StockVO> nasdaq100List =
		 * getUsStockList(nasdaq100, "nasdaq100"); Collections.sort(nasdaq100List, new
		 * DividendRateDescCompare()); writeUsStockList(nasdaq100List, "dowJones");
		 * logger.debug("nasdaq100List.size:" + nasdaq100List.size()); //나스닥 종합지수
		 ...
		 * logger.debug("nasdaqTotalList.size:" + nasdaqTotalList.size()); //다우존스
