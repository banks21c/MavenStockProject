 ...
 * 유튜브에서 네이버로 공유할때
 */
public class NaverStockTradingVolume {

	private static final Logger logger = LoggerFactory.getLogger(NaverStockTradingVolume.class);

	static String strYmdhms = new SimpleDateFormat("yyyyMMdd_hhmmss", Locale.KOREAN).format(new Date());
	public final static String USER_HOME = System.getProperty("user.home");

	public static void main(String args[]) throws InterruptedException, IOException, Exception {
		new NaverStockTradingVolume();
	}

	NaverStockTradingVolume() throws IOException, Exception {
...
//			https://m.stock.naver.com/api/item/getTrendList.nhn?code=251270&size=5
			headers.set("authority", "m.stock.naver.com");
			headers.set("method", "GET");
			headers.set("path", "/api/item/getTrendList.nhn?code=251270&size=5");
...
//			https://m.stock.naver.com/api/item/getTrendList.nhn?code=251270&size=5
			headers.set("authority", "m.stock.naver.com");
			headers.set("method", "GET");
			headers.set("path", "/api/item/getTrendList.nhn?code=" + stockCode + "&size=5");
			headers.set("scheme", "https");
//			headers.set("accept", "*/*");
			headers.set("accept",
					"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9");
...
//			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(SERVER_URI);
			UriComponentsBuilder builder = UriComponentsBuilder.newInstance().scheme("https").host("m.stock.naver.com");
			builder = builder.path("/api/item/getTrendList.nhn");
			builder = builder.queryParam("code", stockCode);
			builder = builder.queryParam("size", "5");
			UriComponents uriComponents = builder.build();
			URI uri = uriComponents.toUri();
			System.out.println("uri:" + uri);
