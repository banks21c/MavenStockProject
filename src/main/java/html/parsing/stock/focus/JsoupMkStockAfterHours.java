 ...
 * 유튜브에서 네이버로 공유할때
 */
public class JsoupMkStockAfterHours {

	private static final Logger logger = LoggerFactory.getLogger(JsoupMkStockAfterHours.class);

	static String strYmdhms = new SimpleDateFormat("yyyyMMdd_hhmmss", Locale.KOREAN).format(new Date());
	public final static String USER_HOME = System.getProperty("user.home");

	public static void main(String args[]) throws InterruptedException, IOException, Exception {
		new JsoupMkStockAfterHours();
	}

	JsoupMkStockAfterHours() throws IOException, Exception {
	...
	public static StockVO getAfterHoursStockTradeInfoVO(StockVO svo) {
		...
		try {
			...
			if (table1Els.size() > 1) {
				...
				if (strAfterHoursVaryPrice.startsWith("▲")||strAfterHoursVaryPrice.startsWith("↑")) {
					strSign = "+";
				...
				} else if (strAfterHoursVaryPrice.startsWith("▼")||strAfterHoursVaryPrice.startsWith("↓")) {
					strSign = "-";
					iCurPrice = iAfterHoursEndPrice + iAfterHoursVaryPrice;
					logger.debug("iCurPrice:" + iCurPrice);
					fVaryRate = (float) iAfterHoursVaryPrice / iCurPrice * 100;
				...
				svo.setStrAfterHoursSign(strSign);
			}
